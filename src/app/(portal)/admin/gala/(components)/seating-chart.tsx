...
export function AdminSeatingChart({ bookings }: AdminSeatingChartProps) {
  const tables = useMemo(() => {
    ...
    return initialTables.map((table): TableStatus => ({
      ...table,
      seats: table.seats.map((seat) => {
        const booking = bookings.find(
          (b) => b.table === table.id && b.seat === seat.number
        );
  ...
  return (
    <Paper withBorder p="md" radius="md" mb="xl">
      <Box mb="md" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Text size="lg" fw={500}>Seating Chart Overview</Text>
