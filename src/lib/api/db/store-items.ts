...
import { StoreItem, StoreItemInput, StoreItemSearchParams } from '@/types/store';
import { NotFoundError, ValidationError, BadRequestError } from '@/lib/api/errors';
 ...
 */
export async function searchStoreItems(params: StoreItemSearchParams): Promise<{ items: StoreItem[]; total: number }> {
  const {
    search = '',
    active,
    limit = 10,
...
export async function createStoreItem(itemData: StoreItemInput): Promise<StoreItem> {
  ...
  if (!itemData.item_code || !itemData.name || itemData.price === undefined || !itemData.description) {
    const missingFields: Record<string, string> = {};
    if (!itemData.item_code) {missingFields.item_code = 'Item code is required';}
    if (!itemData.name) {missingFields.name = 'Name is required';}
    if (itemData.price === undefined) {missingFields.price = 'Price is required';}
    if (!itemData.description) {missingFields.description = 'Description is required';}
 ...
 */
export async function updateStoreItem(id: string, itemData: Partial<StoreItemInput>): Promise<StoreItem> {
